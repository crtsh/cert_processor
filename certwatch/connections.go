...
func init() {
	...
	connectString = "postgresql:///certwatch?host=" + url.QueryEscape(config.Config.CertWatchDB.Host) + "&application_name=cert_processor@" + url.QueryEscape(config.GitCommit) + "&user=" + url.QueryEscape(config.Config.CertWatchDB.User)
