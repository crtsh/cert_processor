...
func init() {
	...
	connectString = "postgresql:///certwatch?host=" + url.QueryEscape(config.Config.CertWatchDB.Host) + "&application_name=cert_processor@" + url.QueryEscape(config.VcsRevision) + "&user=" + url.QueryEscape(config.Config.CertWatchDB.User)
	...
	} else if connCertProcessor, err = pgx.ConnectConfig(context.Background(), pgxConfig); err != nil {
		LogPostgresFatal(err)
...
func Close() {
	n := 0
	if connCertProcessor != nil {
		connCertProcessor.Close(context.Background())
		n++
	}
